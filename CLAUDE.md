...
ride-static/
  ...
  src/                    Stage 2+ — parser, model, render
    ...
    parser/               TEI → domain
    render/, build.py     html, pdf, aggregations, explore, build CLI
                          (refs resolution and asset rewriting live in parser/, not here)
  templates/html/         Jinja templates (incl. explore.html for /data/explore/)
  ...
  knowledge/              Obsidian-style vault, .md only, wikilinks for cross-refs
    ...
    exploration.md        proposal + plan for the interactive explore and story views (status partial)
    redirects-feeds.md    legacy-URL redirects and syndication-feed rationale (hand-written)
    workflow.md           target publication workflow + legacy-stack replacement inventory (hand-written)
...
| Design intent | `architecture.md`, `pipeline.md`, `redirects-feeds.md`, `workflow.md` | hand-written |
| Product specification | `specification.md`, `interface.md`, `exploration.md` | hand-written |
