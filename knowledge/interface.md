...
Die **Rezensionsansicht** ist die Hauptansicht (Abschnitt 5). Die **Factsheet-Vollseite** (R18) ist ein eigener Seitentyp unter `/issues/{N}/{id}/factsheet/`, der den vollständigen Fragebogen pro Frage rendert. Die Seite nutzt eine breitere Einspaltenansicht. Ihr Kopf gruppiert die Publikationsmetadaten, die rezensierte Ressource und die beteiligten Personen in klar abgegrenzten Bereichen. Jede Frage bildet einen eigenen Block mit Kriterienverweis, Fragetext und ausdrücklich bezeichnetem Antwortwert. Review und Factsheet tragen dieselbe Artefaktnavigation mit Review, Factsheet, TEI XML und PDF. Der Hilfetext je Kriterium liegt als natives `<details>`-Toggle direkt an der Frage, Inhalt aus `content/factsheet-help/{se,tc,te}.md` (redigierbares Markdown, geladen über `src/render/factsheet.py`); im Print ist das Toggle ausgeblendet. Freitext-Auswahlantworten (das TEI-`<gloss>` ohne `@type`) erscheinen als „Other: …". Compound Reviews, die mehr als eine Ressource bewerten, tragen pro Fragebogen-Block eine eigene Überschrift je rezensierter Ressource. **Aggregationsseiten** (Tags, Reviewer, Reviewed Resources, Data) tragen eine Sortier- und Filterleiste oben und eine Liste oder Tabelle als Inhalt. Die **Reviewed-Resources-Tabelle** setzt pro Zeile den kanonischen Werktitel als einzigen Link; die verantwortlichen Personen und das Erscheinungsjahr stehen darunter als gedämpfte Credits-Zeile, Rollenlabel und Zugriffsdatum bleiben dem Factsheet vorbehalten. Platzhalter-Namen (redaktionelle N.-N.-Marker) werden aus der Credits-Zeile gefiltert, sodass nur belegte Personen erscheinen. **Editorialseiten** (About, Imprint, Reviewing Criteria plus die acht weiteren editorialen Pages aus den About- und Reviewers-Untermenüs) werden aus TEI gespeist, Quelle ist `pages/<slug>.xml` (validiert gegen das Schema `schema/ride-pages.rng`); gerendert werden sie über dasselbe Template `editorial.html` wie zuvor und verwenden nur die Inhaltsspalte ohne Sidebar.

## 5. Rezensionsansicht im Detail

Der **Kopfbereich** trägt drei Bestandteile in dieser Reihenfolge. Der **Titel** als prominentes Heading-Element. Direkt darunter die **bibliografische Angabe der besprochenen Edition** als eigener Absatz: Werktitel in Italic, Editorenliste mit `(ed.)`, Erscheinungsjahr, URL und Last-Accessed-Datum bei Online-Quellen. Im selben Absatz nahtlos angeschlossen die **Reviewer-Information** in der Form „Reviewed by ⟨Autoritäts-Badge⟩ Vorname Nachname (Affiliation), email@example.org." — Inline-Prosa statt strukturierter Liste, damit die bibliografische und die Reviewer-Information zusammen als ein zusammenhängender Eingangssatz lesbar sind. Das **Autoren-Identifier-Badge** trägt das Label der erkannten Autorität (ORCID, GND oder VIAF) und verlinkt auf die normalisierte URI; ein nicht klassifizierbarer `@ref` (Junk) rendert Text ohne Link. Dieselbe Badge-Logik gilt in Factsheet- und Reviewer-Templates. Strukturelle Auszeichnung (BEM-Klassen, Identifier-Link) bleibt darunter erhalten. Die Reviewer-E-Mail wird als obfuskierter Text-Span ohne `mailto:` gerendert; ein aus der verschleierten Form gebauter `mailto:`-Link wäre eine ungültige URL, deshalb trägt der Span nur den maskierten Anzeigetext.

Das **Abstract** erscheint als leicht hinterlegter Block mit einem eigenen Heading „Abstract", das den Block sowohl visuell als auch semantisch (für Screenreader) öffnet. Das Heading ist ein `h2` (per Stylesheet auf h3-Größe gesetzt), sodass die Dokument-Gliederung vom Review-Titel-`h2` keine Ebene überspringt. Begründung der Heading-Wiederholung: das Heading dient hier nicht der Hierarchie-Information, sondern als deutlicher Funktions-Anker — Leser:innen erkennen den Abstract sofort, Screenreader können in Heading-Listen navigieren.
...
Der **Apparate-Block** (References, Figures, Notes) wird als klar getrennte Sub-Blöcke unter einer gemeinsamen Trennlinie gesetzt, mit eigenem h3-Header pro Sub-Block (Abschnitt 6). Ein vierter Sub-Block „Amendments" tritt hinzu, sobald eine Rezension nachträgliche Korrekturen trägt.
...
2. **Factsheet** — die Vollständigkeit des Fragebogens als beantwortete Fragen gegen vorhandene Fragen, ergänzt um den Link „Full factsheet" auf die Factsheet-Vollseite unter `/issues/{N}/{id}/factsheet/` (R18, §4). Ausgewählte Antwortwerte werden nicht als Qualitätswert interpretiert.
3. **Meta** — Published-Datum, DOI und als letzte Zeile das Lizenz-Kürzel (z.B. „CC BY 4.0", abgeleitet aus der Lizenz-URL). Die Dateizugänge stehen gesammelt in der Artefaktnavigation der Inhaltsspalte.
...
Bidirektionale Verlinkung ist in allen Apparaten Pflicht. Jede Figure-Nummer in der Liste verlinkt zurück zur Bildposition im Text, jede Footnote zur Aufrufstelle, jede Reference auf die Inline-Erwähnung (sofern eindeutig). Das ist heute uneinheitlich und wird normalisiert.

Ein vierter Apparat-Sub-Block **Amendments** tritt zu References, Figures und Notes hinzu, aber nur wenn eine Rezension nachträgliche Korrekturen trägt (`<mod change="#revisionN">`); Rezensionen ohne Korrektur zeigen ihn nicht. Der Block listet je Korrektur die ursprüngliche Fassung unter „Original", die Ersetzung steht als Inline-Austausch bereits im Fließtext, dazu die Notiz der Rezensentin mit Datum. Die Verlinkung ist auch hier bidirektional: der Inline-Marker an der Korrekturstelle und der Apparat-Eintrag verweisen aufeinander. Modell und Parser-Branch in [[architecture#Domain model]].
...
| `--ride-accent` | `#0d6efd` | Links, Querverweise, Identifier-Badges (ORCID/GND/VIAF) |
...
**Keine redundanten Landmark-Rollen.** Der globale `<header>` und `<footer>` auf oberster Ebene tragen keine explizite `role`; die HTML-Elemente liefern die Banner- und Contentinfo-Landmark bereits, ein zusätzliches `role`-Attribut würde sie doppeln. Ebenso werden **keine verschachtelten Anker** erzeugt: ein Bibliografie-Eintrag wird nur dann als Ganzes in einen Link gehüllt, wenn er selbst keinen `<a>` enthält, weil `<a>` in `<a>` ungültiges HTML ist und Browser den äußeren Anker zerlegen.
...
JavaScript ist auf kleine, unabhängige Vanilla-Module beschränkt (Copy-Link, Cite-Kopieraktion, Pagefind-Integration, Navigation, Explore-View), ohne Framework und ohne Bundling-Pipeline. Das hält das Build-Budget überschaubar und passt in den Single-Workflow-Build aus [[specification#N10 Single-Workflow-Build]]. Die Dropdown-Navigation aus Abschnitt 4 wird über `<details>` plus CSS realisiert, weil ein Dropdown-Mechanismus nativ in der Plattform existiert. `nav.js` ergänzt darüber nur das exklusive Öffnen (ein geöffnetes Dropdown schließt die übrigen, Klick außerhalb und Escape schließen alle), denn native `<details>` kennen einander nicht und stapeln sonst offene Untermenüs; ohne JavaScript bleibt das native Verhalten vollständig bedienbar.
